...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	result = InitializeShader(device, hwnd, L"../f4engine1/TextureVertexShader.vs", L"../f4engine1/TexturePixelShader.ps");
	if (!result) {
		return false;
	}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	result = D3DCompileFromFile(vsFilename, 0, 0, "TextureVertexShader", "vs_5_0", dwShaderFlags, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result)) {
	...
	result = D3DCompileFromFile(psFilename, 0, 0, "TexturePixelShader", "ps_5_0", dwShaderFlags, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result)) {
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage) {
